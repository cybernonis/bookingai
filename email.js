...
export async function sendAdminNotification({ to, businessName, bookingNum, name, phone, email, rows }) {
  const transporter = getTransporter();
  ...
  const rowsHtml = rows
    ...
    .map(r => `
      <tr>
        <td style="padding:10px 12px;font-size:18px;width:32px;vertical-align:middle;">${r.icon}</td>
        <td style="padding:10px 6px;font-size:11px;color:#6b7280;white-space:nowrap;vertical-align:middle;text-transform:uppercase;letter-spacing:0.04em;">${esc(r.label)}</td>
        <td style="padding:10px 14px;font-size:13px;font-weight:500;color:#1a1d23;vertical-align:middle;">${esc(String(r.value))}</td>
      </tr>
      <tr><td colspan="3" style="padding:0;height:1px;background:#e5e8ed;"></td></tr>`)
    .join('');
...
<body style="margin:0;padding:0;background:#f4f6f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" role="presentation"
        ...
        <tr>
          <td style="background:#16a34a;padding:28px 32px;">
            <p style="margin:0 0 4px;color:rgba(255,255,255,0.7);font-size:11px;text-transform:uppercase;letter-spacing:0.1em;">New Booking</p>
            <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;line-height:1.3;">${esc(businessName)}</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px 16px;">
            <div style="display:inline-block;padding:10px 18px;background:#f0fdf4;border-left:4px solid #16a34a;border-radius:0 8px 8px 0;margin-bottom:8px;">
              <p style="margin:0;font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:0.06em;">Booking Number</p>
              <p style="margin:4px 0 0;font-size:22px;font-weight:800;color:#16a34a;letter-spacing:0.05em;">${esc(bookingNum)}</p>
            </div>
          </td>
        </tr>
        <tr>
  ...
  try {
    const info = await transporter.sendMail({
      ...
      to,
      subject: `🆕 New Booking ${bookingNum} — ${name}`,
      html,
    });
    console.log(`Admin email sent to ${to} (${bookingNum}) — ${info.messageId}`);
  } catch (err) {
